...
class RemoveTeacherFromFavoriteUseCaseTest {
    ...
    @InjectMocks
    private RemoveTeacherFromFavoriteUseCase useCase;

    @Test
    void givenTeacherInFavoriteList_whenExecute_thenShouldInvokeUpdateMethod() {
        when(service.listFavoriteTeachers(STUDENT_ID)).thenReturn(NON_EMPTY_TEACHER_LIST);
        useCase.execute(STUDENT_ID, TEACHER_ID);
        verify(service).updateFavoriteTeachers(STUDENT_ID, Collections.emptyList());
    }

    @Test
    void givenTeacherNotInFavoriteList_whenExecute_thenShouldThrowException() {
